...
public class Deck : MonoBehaviour {
	...
	public void MakeCards() {
		...
		foreach (string s in letters) {
			for (int i=0; i<13; i++) { // for loop should only go up to 13, not 14
				cardNames.Add(s+(i+1));
			}
		}
    ...
    public void shuffleDeck(ref List<Card> unshuffledDeck) // Deck shuffling function, made public void to be called by Prospector.cs
    {
        List<Card> shuffledDeck = new List<Card>(); // make a new target deck which will be shuffled
        int indexNum;
        while (unshuffledDeck.Count > 0) // do until the unshuffled deck is empty and all cards are in the shuffled target deck
        {
            indexNum = Random.Range(0, unshuffledDeck.Count); // pick a random card in the unshuffled deck
            shuffledDeck.Add(unshuffledDeck[indexNum]); // add this card to the shuffled deck
            unshuffledDeck.RemoveAt(indexNum); // remove this card from the unshuffled deck
        }
        unshuffledDeck = shuffledDeck; // set the unshuffled deck to the shuffled deck, so it now points to a shuffled deck.
    }
